...
char * convNMEA(char *nmea_msg)
{
    ...
    //char * nmea_msg;
    ////char nmea_msg[100];
    char nmtype[7], nmts[11], nmav[2], nmla[10], nmns[10], nmlo[10], nmew[10], nmsp[10], nmtc[10], nmea_date[7], nmcrc[5];
    ...
    if (!strcmp(nmtype,"$GPRMC") && !strcmp(nmav,"A"))    // strcmp returns 0 if strings match!
    {
        ////printf("Valid $GPRMC detected! --> %s %c\n", nmea_date, nmcrc[0]);
        ...
        int crc;
        //fprintf(stdout, "FIRST:%i <--> ", strlen(nmea_msg));
	    sprintf(nmea_msg, "%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,,,%c", nmtype, nmts, nmav, nmla, nmns, nmlo, \
                                                                nmew, nmsp, nmtc, str, nmcrc[0]);
        //fprintf(stdout, "SECOND:%i <--> NMCRC[0]: ->%c<-\n", strlen(nmea_msg), nmcrc[0]);
        //nmea_msg[strlen(nmea_msg)] = '\0';
        //nmea_msg[strlen(nmea_msg)+1] = '\0';
        //nmea_msg[strlen(nmea_msg)+2] = '\0';
        //nmea_msg[strlen(nmea_msg)+3] = '\0';
        //fprintf(stdout, "CRC: started...\n");
	    crc = nmea0183_checksum(nmea_msg);
        //fprintf(stdout, "CRC: ended. ->%s<-\n", nmea_msg);
	    sprintf(nmea_msg, "%s*%X", nmea_msg, crc);
        //fprintf(stdout, "CRC: ->%X<- Length: ->%i<-\n->%s<_-_-_\n", crc, strlen(nmea_msg), nmea_msg);
        //realloc(nmea_msg, strlen(nmea_msg)+2);
        nmea_msg[strlen(nmea_msg)+1]=0;
        nmea_msg[strlen(nmea_msg)]='\n';

        //fprintf(stdout, "CRC: ->%X<- Length: ->%i<-\n->%s\n<-", crc, strlen(nmea_msg), nmea_msg);
        //fprintf(stdout, "CRC: ->%X<- Length: ->%i<-\n", crc, strlen(nmea_msg));
        //fprintf(stdout, "New NMEA message:%s<->AAAAAA<----\n", nmea_msg);
